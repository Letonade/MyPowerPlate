	...
	<body>
		<div class="container">
			<div class="py-5 text-center">
					<h2><?=MY_APP_NAME?></h2>
					<!-- #NoExam tous les <<surplus>> sont aux "#NoExam". Penser à retirer cette ligne pour l'éxam ! -->
					<!-- Dans sublime ctrl+shift+f permet de chercher du texte dans tous les fichier du projet. -->
					<div class="badge badge-secondary"> Le compte admin c'est juste admin/admin les gars !.</div>
		      </div>
			<div class="row">
				<div class="py-2 col-md-12 login-sec">
					<form id="formulaire" class="login-form" action="" method="post" onkeyup='if (event.keyCode === 13) {document.getElementById("formulaire_validate").click();}'>
						...
						<button id="formulaire_validate" type="button" class="btn btn-primary btn-lg btn-block" onclick="fct_verifier_connexion('verification')">Log In</button>
					</form>
				</div>
			</div>
		</div>
